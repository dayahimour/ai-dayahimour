...
- **sourceUrl:** (اختياري) يمكنك تركه فارغاً `""` — **الروابط الخارجية أصبحت معطلة**. عند الضغط على الخبر لن يذهب المستخدم للرابط الخارجي.
...
- **articleSlug:** ⚠️ **مهم جداً** — لربط الخبر بمقالة داخلية على الموقع. ضع هنا الـ slug الخاص بالمقالة (مثال: `"claude-opus-4-7-analysis"`). عندها فقط يصبح الخبر قابلاً للنقر وينقل القارئ للمقالة الداخلية. **بدون articleSlug، الخبر يظهر للقراءة فقط بدون رابط.**
- **draft:** دائماً `false`

### كيفية ربط الخبر بمقالة داخلية:
1. إذا كان الخبر يستحق مقالة تفصيلية على موقعك، اكتب slug المقالة في `articleSlug`
2. المقالة يجب أن تكون موجودة في `src/content/blog/` (عربي) أو `src/content/blog-en/` (إنجليزي)
3. slug = اسم ملف المقالة بدون `.md` (مثال: ملف `claude-opus-4-7-analysis.md` → slug: `"claude-opus-4-7-analysis"`)
4. عند وجود `articleSlug`، يظهر الخبر بلون أزرق ويكون قابلاً للنقر
5. بدون `articleSlug`، يظهر الخبر بلون رمادي ولا يفعل شيئاً عند الضغط عليه
...
⚠️ تذكير مهم: الروابط الخارجية معطلة. الخبر يصبح قابلاً للنقر فقط عند ربطه بمقالة داخلية عبر articleSlug.
...
📌 articleSlug — ضع slug المقالة الداخلية فقط إذا كان للخبر مقالة على الموقع. أمثلة:
   - "openai-gpt-5-analysis"
   - "google-gemini-new-features"
   - "claude-4-release-review"

بدون articleSlug → الخبر للقراءة فقط (لا يفعل شيئاً عند الضغط)
مع articleSlug → الخبر قابل للنقر وينقل للمقالة الداخلية
