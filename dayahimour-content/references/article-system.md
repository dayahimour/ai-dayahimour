...
### مقالات عربية (category) — **يجب أن تطابق هذه الكلمات بالضبط**
| القيمة | الاستخدام |
|--------|-----------|
| `نماذج` | مقالات عن نماذج AI لغوية / متعددة الوسائط |
| `أدوات` | أدوات وبرامج AI |
| `شركات` | شركات AI وتقارير عنها |
| `تحليل` | تحليلات عميقة |
| `أخبار` | أخبار مطوّلة |
| `فيديو` | نماذج توليد فيديو |
| `صور` | نماذج توليد صور |
| `مستقبل` | توقعات مستقبلية |
| `مقارنات` | مقارنات بين نماذج أو أدوات |

### مقالات إنجليزية (category) — **أحرف صغيرة فقط — لا تكبير أول حرف**
| القيمة | الاستخدام |
|--------|-----------|
| `models` | AI language / multimodal models |
| `tools` | AI tools and software |
| `companies` | AI companies and reports |
| `analysis` | Deep analysis articles |
| `news` | Extended news articles |
| `video` | Video generation models |
| `images` | Image generation models |
| `future` | Future predictions |
| `comparisons` | Model/tool comparisons |

> ⚠️ **خطأ شائع**: كتابة `"Models"` بدلاً من `"models"` — المخطط (schema) يقبل **أحرف صغيرة فقط** ويرفض أي حرف كبير.
> ⚠️ **خطأ شائع**: كتابة `"model"` مفرد بدلاً من `"models"` جمع — المخطط يقبل الجمع فقط.
...
قبل إنهاء أي مقالة — **اقرأ frontmatter مرة أخيرة** وتأكد من:

| # | الحقل | القاعدة |
|---|-------|---------|
| 1 | `category` | العربي: **أحرف عربية** (`نماذج`، `أدوات`...) — الإنجليزي: **أحرف صغيرة** (`models`، `tools`...) |
| 2 | `slug` | إنجليزي صغير، كلمات بشرطة، **بدون سنة** |
| 3 | `image` | المسار `/images/[slug].webp` — تأكد من تطابق الـ slug |
| 4 | `tier` | فقط عند `category: "نماذج"` أو `"models"` — القيم: `flagship` / `standard` / `budget` |
| 5 | `date` | صيغة `"YYYY-MM-DD"` — تأكد من تطابق تاريخ اليوم |
| 6 | `description` | بين 150-160 حرفاً، تحتوي الكلمة المفتاحية |
| 7 | `title` | بين 50-60 حرفاً |
| 8 | أشرطة HTML | `width: XX.X%` يطابق الرقم المكتوب — **لا تفريغ بيانات** |
| 9 | أشرطة HTML | لا تترك شريطاً فارغاً — احذف المعيار غير الموجود كاملاً |
| 10 | Benchmarks | كل رقم موجود في مصدر موثوق (لا اختراع) |

> 🛑 **أهم خطأين شائعين:**
> - `"Models"` بدلاً من `"models"` → **فشل البناء كامل**
> - `"model"` (مفرد) بدلاً من `"models"` (جمع) → **فشل البناء كامل**
